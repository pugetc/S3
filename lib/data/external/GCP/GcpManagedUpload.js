...
class GcpManagedUpload {
    ...
    send(callback) {
        ...
        if (this.body instanceof stream) {
            ...
            .on('data', data => this.chunkStream(data))
            .on('end', () => {
    ...
    processChunk(chunk) {
        ...
        ++this.activeParts;
        if (this.activeParts >= this.queueSize && !this.body.isPaused()) {
            // pause stream if current active uploads is greater than
            // or equal to queue size
            this.body.pause();
        }
        if (!this.uploadId) {
