...
function batchDelete(request, response, log, callback) {
    ...
    request.on('end', () => {
        let locations;
        try {
            locations = JSON.parse(Buffer.concat(data), totalLength);
        } catch (e) {
            // FIXME: add error type MalformedJSON
            return callback(errors.MalformedPOSTRequest);
        }
        console.log('BATCH DELETE LOCATIONS', locations);
        // FIXME: replace current batchDelete call with a reliable
        // implementation (i.e with retry capability and flow control,
        // at least)
        data.batchDelete(locations, 'DELETE', storageLocation, log);
...
function routeBackbeat(clientIP, request, response, log) {
    ...
    // FIXME rework this
    if (backbeatRoutes[request.method][request.resourceType]
        === batchDelete) {
        return batchDelete(request, response, log, err => {
            if (err) {
